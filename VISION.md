...
| Layer | Choice | Why |
|-------|--------|-----|
| Web framework | **Next.js 14+ (App Router), React, TypeScript** | Already live; RSC/ISR serve static curriculum beautifully; one codebase for web, API, and BFF |
...
| Auth | **Auth.js (NextAuth v5)** | Email/password + Google/GitHub OAuth; free and ecosystem-native |
...
**What stays from today's build:** Next.js App Router, the modular 100-day curriculum
engine, the lazy-loaded lesson modules, the in-browser C/ASM simulator, the token-less
Piston fallback, and the performance discipline (self-hosted fonts, CDN'd editor,
optimized bundles). These are not being replaced — they are the seed the stack grows from.
